...
class ExplainModule(nn.Module):
    ...
    def loss(self, pred, pred_label, node_idx, epoch):
        ...
        L = D - m_adj
        if self.graph_mode:
            pred_label_t = torch.as_tensor(pred_label, dtype=torch.float)
        else:
            pred_label_t = torch.as_tensor(pred_label, dtype=torch.float)
            if pred_label_t.dim() == 0: # scalar
                pred_label_t = pred_label_t.unsqueeze(0) # [1]
            if pred_label_t.dim() == 1:
                # If it's a vector [N], we need [N, 1] for matrix mult? 
                # Original logic: pred_label_t @ L @ pred_label_t
                # If L is [N, N], pred_label_t should be [N].
                # PyTorch @ handles vector-matrix-vector correctly.
                pass
        if self.args.gpu:
