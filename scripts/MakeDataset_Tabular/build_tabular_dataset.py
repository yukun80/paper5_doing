...
def main():
    ...
    if not features_path.exists() or not labels_path.exists():
        raise FileNotFoundError(f"Missing input files for mode {args.mode}. Please run scripts/00_common/ with --mode {args.mode} first.")
    ...
    if not insar_files or not su_grid_path.exists():
        ...
        with rasterio.open(insar_path) as src_insar, rasterio.open(su_grid_path) as src_su:
            ...
            stats = df_insar.groupby("su_id")["val"].agg(
                mean_vel=lambda x: np.mean(x),
                top20_abs_mean=lambda x: get_top20_mean(x)
            )
