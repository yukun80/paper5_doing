...
:: CONFIGURATION: STATIC CONTROL (Pre-fire Indices)
:: ==============================================================================
...
python scripts/00_common/10_build_raster_stack.py --config metadata/dataset_config_static.yaml --mode static
if %ERRORLEVEL% NEQ 0 (echo [ERROR] Failed in Raster Stack Builder && exit /b %ERRORLEVEL%)

echo 2. Extracting Slope Unit Features...
python scripts/00_common/20_extract_su_features.py --config metadata/dataset_config_static.yaml --mode static
if %ERRORLEVEL% NEQ 0 (echo [ERROR] Failed in Feature Extraction && exit /b %ERRORLEVEL%)

echo 3. Generating Labels...
python scripts/00_common/30_generate_labels.py --config metadata/dataset_config_static.yaml --mode static
if %ERRORLEVEL% NEQ 0 (echo [ERROR] Failed in Label Generation && exit /b %ERRORLEVEL%)

echo 4. Building Graph Topology...
python scripts/00_common/40_build_graph.py --config metadata/dataset_config_static.yaml --mode static
...
python scripts/MakeDataset_Tabular/build_tabular_dataset.py --config metadata/dataset_config_static.yaml --mode static
...
echo ============================================================
echo Training RF...
python "experiments/01_ml_baselines/random forest/train_rf.py" --mode static
echo Inference and Mapping RF...
python "experiments/01_ml_baselines/random forest/inference_rf.py" --mode static
...
echo ============================================================
echo Training SVM...
python experiments/01_ml_baselines/svm/train_svm.py --mode static
echo Inference and Mapping SVM...
python experiments/01_ml_baselines/svm/inference_svm.py --mode static
...
if %ERRORLEVEL% EQU 0 (
    ...
    python "experiments/01_ml_baselines/xgboost/train_xgb.py" --mode static
    echo Inference and Mapping XGBoost...
    python "experiments/01_ml_baselines/xgboost/inference_xgb.py" --mode static
) else (
...
python experiments/02_dl_gcn/train_gcn.py --mode static
...
python experiments/GNNExplainer/train_landslide.py --mode static
...
python experiments/GNNExplainer/explain_landslide.py --mode static
